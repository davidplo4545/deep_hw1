...
class BiasTrick(object):
    ...
    def __call__(self, x: torch.Tensor):
        ...
        #  Hint: See torch.cat().
        # ====== YOUR CODE: ======
        ones_tensor = torch.ones_like(x[..., :1])

        # Concatenate ones_tensor with tensor along the specified dimension
        tensor_with_bias = torch.cat((ones_tensor, x), dim=-1)
